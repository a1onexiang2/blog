...
#### 贪婪算法
贪婪即在局部作出在最好的选择。虽然贪心算法不能对所有问题都得到整体最优解，但对许多问题它能产生整体最优解。如单源最短路经问题，最小生成树问题等。在一些情况下，即使贪心算法不能得到整体最优解，其最终结果却是最优解的很好近似。
- **会议安排问题**
...
- **数字组合问题**
给定一组数字 {a<sub>1</sub>, a<sub>2</sub>, …, a<sub>n</sub>}，现以拼接字符串的方式组合这些数字使之成为一个大数字，如何使这个数字最大。
贪心策略：求出每个数字的最高位（1~9），根据最高位的大小（若最高位相同，则比较第二高位数字，直到最后一位为止，最高位以外没有数字时视为最大）拼接数字。
- **找零钱的问题**
...
用两个栈，数值栈 a 与运算符栈 b 来实现。
    ...
    4. 遇到左括号时压入 b。
    5. 遇到右括号时执行运算操作直至 b 弹出左括号。
- **栈能否弹出特定数列**
文字表述：
问题：一个栈的输入是数列 A，假定输出是数列 B，如何判断 B 是否能通过入栈出栈操作从 A 转换而来。
解答：令原索引指 B 中的值在 A 中的索引位置，对 B 的每一个值，其后所有原索引比他小的值，它们的原索引必须符合降序排序。
数学表述：
